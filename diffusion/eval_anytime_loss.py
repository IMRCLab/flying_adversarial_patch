...
if __name__ == '__main__':
    ...
    anytime_losses_gt = np.array([np.load(file_path) for file_path in file_paths_gt[random_idx]])
    ...
    anytime_losses_diff = np.array([np.load(file_path) for file_path in file_paths_diff[random_idx]])
    ...
    axs.axvline(x=mean_timestamps_gt[5], alpha=0.4, linestyle='--')
    axs.axvline(x=mean_timestamps_diff[5], alpha=0.4, color='#ff7f0e', linestyle='--')
    ...
    fig.savefig(f'eval/eval_anytime_loss_3e.pdf', dpi=200)